...
- **`templates/index.ts`** is a registry of `TemplateDefinition`s (`id`, `name`, `defaultAccentId`, `supportsAccent?`, `Component`). Templates are `Duo`, `Linea`, `Aria`, `Mono`. Each receives `TemplateProps = { resume, accent, marginScale }`. `Mono` is monochrome by design and sets `supportsAccent: false`, which disables the accent picker while it is active.

...
├── resume-templates/   # PDF template components
│   ├── basic.template.tsx
│   └── duo.tsx
...
### State Management (`resume.context.tsx`)
...
### Skills Section (`components/editor/skills-section/`)
...
### Resume Context (`context/resume.context.tsx`)
...
### Reducer Actions (`context/resume.reducer.ts`)
```typescript
enum ACTIONS {
...
## PDF Template System (`resume-templates/`)

### Duo Template (`duo.tsx`)
...
Note: While the original CLAUDE.md mentioned Firebase, the current codebase has moved away from Firebase backend services. The application now uses:
- **localStorage**: For data persistence
- **Client-side only**: No backend dependencies
- **Static deployment**: Suitable for GitHub Pages or similar static hosts

The Vite configuration still includes a proxy for Firebase functions (`/api/exportPDF`), but this appears to be legacy code that may not be currently used.
